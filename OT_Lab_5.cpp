...
int main(){
    ...
    //coeffiecient vector
    vector<vector <double> > A(eqns,vector<double> (vars+eqns+free_var+artificial_vars,0));
    //constraints vector
    vector<double> b(eqns,0);
    //objective function vector
    vector<double> c(eqns+vars+free_var+artificial_vars,0);
    ...
    int temp = 0;
    for(int i = 0; i+vars+free_var < vars+eqns+free_var+artificial_vars; i++){
        if(eqns_type[i-temp] == 's'){
            A[i-temp][i+vars+free_var] = 1;
        }else if(eqns_type[i-temp] == 'g'){
            A[i-temp][i+vars+free_var] = -1;
            A[i-temp][i+vars+free_var+1] = 1;
            c[i+vars+free_var+1] = 100000;
            i++;
            temp++;
        }else if(eqns_type[i-temp] == 'e'){
            A[i-temp][i+vars+free_var] = 1;
            c[i+vars+free_var] = 100000;
        }
    }
    
    cout<<"\nEntered function : \n";
    for(int i = 0; i < eqns + vars + free_var + artificial_vars; i++){
        cout<<c[i]<<"\t";
    }

    cout<<"\n\nA : \n";
    for(int i = 0; i < eqns; i++){
        for(int j = 0; j < eqns+vars+free_var + artificial_vars; j++){
            cout<<A[i][j]<<"\t";
        }
        cout<<"\n";
    }
