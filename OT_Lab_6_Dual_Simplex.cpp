...
#define BigM false  // true for Big M, false for simplex
...
class Simplex{
    ...
    public:
        ...
        bool DualSimplexAlgoritmCalculation(){
            if(checkOptimality() == true && checkForDualSimplex() == false)
                return true;
            ...
            int pivotColumn = findDualpivotColumn(pivotRow);
        ...
        void doDualIterations(){
            bool end = false;
            int iteration = 0;
            while(!end){
                bool result = true;
                bool optimal = checkOptimality();
                if(checkForDualSimplex() == true){
                    result = DualSimplexAlgoritmCalculation();
                }
                if(optimal == true && result == true){
                    end = true;
                }else if(optimal != true && result == true){
        ...
        int findDualpivotRow(){
            int neg_count = 0, max_neg_index = -1, max_neg = 0;
            for(int i = 0; i < rows; i++){
                if(B[i] < 0){
                    neg_count ++;
                    if(B[i] < max_neg){
        ...
        int findDualpivotColumn(int pivot_Row){
            int pivot_Column = -1;
            double minRatio = INT_MAX, ratio;
            for(int i = 0; i < cols; i++){
                if(ZC[i]!=0){
                    ratio = (double)B[pivot_Row]/ZC[i];
                    if(ratio < minRatio){
                        minRatio = ratio;
                        pivot_Column = i;
                    }
        ...
        void Solve(){
            ...
            cout<<"\n";
            // doIterations();
            // int pivot_Row = checkForDualSimplex();
            // if(pivot_Row == false){
            //     cout<<"All values of B are >0, so no Dual Simplex Required";
            // }else{
            //     doDualIterations();
            // }
            doDualIterations();
...
int main(){
    ...
    cout<<"\nEnter equation type (eg eq1 : <= eq2 : >= , input : s g) : ";
    vector<char> eqns_type(eqns,0);
    int artificial_vars = 0;
    temp = 0;
    for(int i = 0; i < eqns; i++){
        cin>>eqns_type[i];
        if(eqns_type[i] != 's' && eqns_type[i] != 'g'){
            cout<<"Invalid input\n";
            return 0x00a;
        }
        if(eqns_type[i] == 'g'){
    ...
    Simplex simplex(eqns, vars, A, b, c, free_var_pos, free_var_count, var_type, 0);
    simplex.print();
    simplex.Solve();
}